#ifndef __LOOSECHARSTRAT_HPP__
#define __LOOSECHARSTRAT_HPP__
...
class LooseCharStrat : public InterpreterStrat{ //similar to multichar strat but the difference is that it still runs if one token matches. executes off of whitespace

        private:
                bool status = false;
		std::string character;
        public:
		LooseCharStrat(std::string characters){this->character = characters;}
			...
			if (token.size() == character.size()){
				...
				if (isValid == true){ //if valid but not all of the needed token
				    this->set_status(true);
				    calls.push_back("09 " + character); //override the current token with the complete version of the character so the interpreter can treat the strategy accordingly
				} else { //if invalid
